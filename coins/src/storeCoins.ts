...
export default async function handler() {
  ...
  try {
    await PromisePool.withConcurrency(5)
      ...
      .process(async (i: any) => {
        try {
          if (!["uniV3"].includes(a[i][0])) return;
          console.log(`Running adapter: ${a[i][0]}`);

          const results = await withTimeout(timeout, a[i][1][a[i][0]](timestamp));
          const filteredResults = results.flat().filter((c: any) => c.symbol != null || c.SK != 0);
          // Only print info, no DB writes
          console.log("Filtered items:", filteredResults.length);
          console.log(`${a[i][0]} done`);
        } catch (e) {
          console.error(
            `${a[i][0]} adapter failed ${process.env.LLAMA_RUN_LOCAL ? "" : `:${e}`}`,
          );
